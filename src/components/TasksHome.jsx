...
export default function TasksHome() {
    ...
    const fetchTasks = async () => {
        try {
            setLoading(true);
            const res = await axios.get("http://127.0.0.1:5000/teacher-tasks");
    ...
    const handleSaveUpdate = async () => {
        try {
            await axios.patch(`http://127.0.0.1:5000/teacher-tasks/${selectedTask._id}`, {
    ...
    const handleConfirmDelete = async () => {
        try {
            await axios.delete(`http://127.0.0.1:5000/teacher-tasks/delete?id=${selectedTask._id}`);
